...
const startRegistrar = async (zoe, installations) => {
  ...
  return { registrarCreatorFacet, registrarInstance };
};

const createCommittee = async (registrarCreatorFacet, voterCreator) => {
  const invitations = await E(registrarCreatorFacet).getVoterInvitations();
...
const makeBootstrap = (argv, cb, vatPowers) => async (vats, devices) => {
  ...
  const { registrarCreatorFacet, registrarInstance } = await startRegistrar(
    zoe,
    installations,
  );

  const votersP = createCommittee(registrarCreatorFacet, voterCreator);
