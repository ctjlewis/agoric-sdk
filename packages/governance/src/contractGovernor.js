...
const validateQuestionDetails = async (zoe, registrar, details) => {
  const {
    counterInstance,
...
const validateQuestionFromCounter = async (zoe, registrar, voteCounter) => {
  const counterPublicP = E(zoe).getPublicFacet(voteCounter);
  const questionDetails = await E(counterPublicP).getDetails();

  return validateQuestionDetails(zoe, registrar, questionDetails);
};
 ...
 *
 * The terms for this contract include the Timer, Registrar and
...
const start = async (zcf, privateArgs) => {
  ...
  const { registrarCreatorFacet } = privateArgs;

  const augmentedTerms = harden({
    ...governedTerms,
    electionManager: zcf.getInstance(),
  });
  const poserInvitation = E(registrarCreatorFacet).getPoserInvitation();
  ...
  assert(
    invitationDetails.instance === registrarInstance,
    X`questionPoserInvitation didn't match supplied Registrar`,
  );
  ...
  const validateRegistrar = async regP => {
    return E.when(regP, reg => {
      assert(
        reg === registrarInstance,
        X`Registrar doesn't match my Registrar`,
      );
      return true;
    });
  };
